...
#bu fonksiyonda farklı embedding modelleri kullanılarak metin verilerinin gömüleri çıkarılır ve kaydedilir
# model_name: kullanılacak embedding modeli
# prefix: dosya adı için ön ek
# file: embedding çıkarılacak dosya adı için (örneğin, train, test)
...
def embed_model(model_name, prefix):
    ...
    for file in ["train","test"]:
        ...
        np.save(f"embeddingsTumveri/{prefix}_{file}_s.npy", s)
        ...
        np.save(f"embeddingsTumveri/{prefix}_{file}_g.npy", g)
        del g
        ...
        np.save(f"embeddingsTumveri/{prefix}_{file}_d.npy", d)
        del d
